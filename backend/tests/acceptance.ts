...
async function main() {
  ...
  await runTest('登录 - 回收站账号', async () => {
    const { statusCode, data } = await request(
      makeOptions('/auth/login', 'POST'),
      { username: 'recycler1', password: '123456' }
    );
    if (statusCode !== 200 || !data.success) {
      throw new Error(data.error || '登录失败');
    }
    recyclerToken = data.data.token;
    if (!recyclerToken) throw new Error('未获取到令牌');
  });

  await runTest('登录 - 乡镇审核员账号', async () => {
    const { statusCode, data } = await request(
      makeOptions('/auth/login', 'POST'),
      { username: 'auditor1', password: '123456' }
    );
    if (statusCode !== 200 || !data.success) {
      throw new Error(data.error || '登录失败');
    }
    auditorToken = data.data.token;
    if (!auditorToken) throw new Error('未获取到令牌');
  });

  await runTest('登录 - 财政复核员账号', async () => {
    const { statusCode, data } = await request(
      makeOptions('/auth/login', 'POST'),
      { username: 'finance1', password: '123456' }
    );
    if (statusCode !== 200 || !data.success) {
      throw new Error(data.error || '登录失败');
    }
    financeToken = data.data.token;
    if (!financeToken) throw new Error('未获取到令牌');
  });

  await runTest('获取农户列表', async () => {
    const { statusCode, data } = await request(
      makeOptions('/farmers', 'GET', recyclerToken)
    );
    if (statusCode !== 200 || !data.success) {
  ...
  await runTest('场景1: 提交无照片批次 - 应该被拒绝', async () => {
    const { statusCode: createCode, data: createData } = await request(
      makeOptions('/batches', 'POST', recyclerToken),
      {
        farmerId,
        weight: 50,
    ...
    const { statusCode: submitCode, data: submitData } = await request(
      makeOptions('/batches/submit', 'POST', recyclerToken),
      { batchId }
    );
    if (submitCode !== 400) {
  ...
  await runTest('为异常批次上传模拟照片', async () => {
    ...
    const { statusCode, data } = await new Promise<{ statusCode: number; data: any }>((resolve, reject) => {
      const req = http.request(options, (res) => {
        let d = '';
        res.on('data', (c) => (d += c));
        res.on('end', () => {
  ...
  await runTest('验证财政复核员可以看到二次复核批次', async () => {
    const { statusCode, data } = await request(
      makeOptions('/reviews/pending', 'GET', financeToken)
    );
    if (statusCode !== 200 || !data.success) {
      throw new Error(data.error || '查询待审核失败');
    }
    const hasSecondReview = data.data.some((b: any) => 
      b.id === testBatchId && b.status === 'SECOND_REVIEW'
    );
    if (!hasSecondReview) {
  ...
  await runTest('乡镇审核员审核通过', async () => {
    const { statusCode, data } = await request(
      makeOptions('/reviews/review', 'POST', auditorToken),
      {
        batchId: rejectedBatchId,
        opinion: '测试审核通过',
        isPassed: true,
  ...
  await runTest('财政复核 - 退回批次', async () => {
    const { statusCode, data } = await request(
      makeOptions('/reviews/review', 'POST', financeToken),
      {
        batchId: rejectedBatchId,
  ...
  await runTest('场景4: 尝试给退回记录发放补贴 - 应该被拒绝', async () => {
    const { statusCode, data } = await request(
      makeOptions('/payments/pay', 'POST', financeToken),
      { batchId: rejectedBatchId, remark: '测试发放' }
    );
    if (statusCode !== 400) {
  ...
  await runTest('验证监管看板数据', async () => {
    const { statusCode, data } = await request(
      makeOptions('/supervisor/dashboard', 'GET', financeToken)
    );
    if (statusCode !== 200 || !data.success) {
